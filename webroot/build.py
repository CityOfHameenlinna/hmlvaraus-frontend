...
SPA_DIR = BASE_DIR + 'varaus-frontend/webroot/'
...
CSS_DIR_PROD = BASE_DIR + 'varaus-frontend-built/webroot/css/'
JS_DIR_PROD = BASE_DIR + 'varaus-frontend-built/webroot/js/'
IMG_DIR_PROD = BASE_DIR + 'varaus-frontend-built/webroot/img/'
FONT_DIR_PROD = BASE_DIR + 'varaus-frontend-built/webroot/fonts/'
TEMPLATE_DIR_PROD = BASE_DIR + 'varaus-frontend-built/webroot/'

INDEX_PROD = TEMPLATE_DIR_PROD + 'index.html'
INDEX_DEV = SPA_DIR + 'index.html'

ERROR_PROD = TEMPLATE_DIR_PROD + '404.html'
ERROR_DEV = SPA_DIR + '404.html'
...
shutil.copy(ERROR_DEV, ERROR_PROD)
...
index_data = index_data.replace('css/main.css', '/venepaikat/css/' + css_md5_filename)
index_data = index_data.replace('js/libs/require.js', '/venepaikat/js/' + js_md5_filename)
index_file = open(INDEX_PROD, 'wb')
index_file.write(index_data)
index_file.close()

error_file = open(ERROR_PROD, 'rb')
error_data = error_file.read()
error_file.close()
error_data = error_data.replace('@import "css/bootstrap.css";', '')
error_data = error_data.replace('@import "css/main.css";', '@import "css/' + css_md5_filename + '";')
error_file = open(ERROR_PROD, 'wb')
